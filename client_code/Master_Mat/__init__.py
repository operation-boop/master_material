...
class Master_Mat(Master_MatTemplate):
  ...
  def button_create_new_click(self, **event_args):
    ...
    try:
      ...
      self.label_result.text = f"✅ Created NEW: {result['document_id']} (Version {result['ver_num']})"
  ...
  def button_create_version_click(self, **event_args):
    ...
    try:
      result = anvil.server.call('create_new_version', self.current_document_id)
      self.label_result.text = f"✅ Created VERSION: {result['document_id']} (Version {result['ver_num']})"
    except Exception as e:
      self.label_result.text = f"❌ Error: {str(e)}"
  ...
  def button_get_versions_click(self, **event_args):
    ...
    try:
      versions = anvil.server.call('get_material_versions', self.current_document_id)
      version_list = "\n".join([f"• Version {v['ver_num']} (ID: {v['id']})" for v in versions])
      self.label_result.text = f"📋 Versions of {self.current_document_id}:\n{version_list}"
    except Exception as e:
      self.label_result.text = f"❌ Error: {str(e)}"
  ...
  def button_get_latest_click(self, **event_args):
    ...
    try:
      latest = anvil.server.call('get_latest_version', self.current_document_id)
      if latest:
        self.label_result.text = f"🎯 LATEST: {latest['document_id']} (Version {latest['ver_num']})"
      else:
