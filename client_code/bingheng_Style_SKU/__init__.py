...
def btn_refresh(self, **event_args):
  ...
  def btn_add(self, **event_args):
    ...
    price = None
    try:
      ptxt = getattr(self, "text_box_price", None) and (self.text_box_price.text or "").strip()
      price = float(ptxt) if ptxt else None
    except Exception:
      alert("Price must be a number"); return

    if not sku_id:
      alert("SKU ID is required"); return
    ...
    try:
      ...
      if isinstance(res, dict) and res.get("ok"):
        ...
        # clear inputs
        self.text_box_sku_id.text = ""
        if hasattr(self, "text_box_ref_id"): self.text_box_ref_id.text = ""
